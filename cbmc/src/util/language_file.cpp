...
bool language_filest::typecheck(symbol_tablet &symbol_table)
{
  ...
  for(filemapt::iterator it=filemap.begin();
      it!=filemap.end(); it++)
  {
    if(it->second.language->interfaces(symbol_table)) //函数体是return false
      return true;
  }
      ...
      it!=filemap.end(); it++)
  {
    if(it->second.modules.empty())
      if(it->second.language->typecheck(symbol_table, "")) //一般没有进这里面
        return true;
  }
  ...
  for(modulemapt::iterator it=modulemap.begin();
      it!=modulemap.end(); it++)
  {
    if(typecheck_module(symbol_table, it->second))//typecheck
      return true;
  }
  ...
  language_modulet &module)
{
  ...
  if(module.file->language->typecheck(symbol_table, module.name))//typecheck
  {
    module.in_progress=false;
    return true;
  }
