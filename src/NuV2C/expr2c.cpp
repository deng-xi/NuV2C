...
#include "../../cbmc/src/ansi-c/c_types.h"
#include "../../cbmc/src/ansi-c/expr2c_class.h"
...
std::string expr2ct::convert_update(
    ...
    forall_operands(it, designator)dest += convert(*it);
...
std::string expr2ct::convert_code_switch(
    ...
    forall_operands(it, src) {
            ...
            if (op.get(ID_statement) != ID_block) {
                ...
                forall_operands(it2, op)dest += convert_code(to_code(*it2), indent + 2);
            }
        }
...
//            +src.op0().op1().get_string(ID_value);
    ...
    dest += "; ";
    if (!src.op2().is_nil())
        dest += convert(src.op2()); //回退修改底层for循环
...
std::string expr2ct::convert(
    ...
    return dest;
    ...
    else if (src.id() == ID_reduction_and) { //增加约简操作
        if (src.op0().find(ID_value) != get_nil_irep()) {
            int n = std::stoi(src.op0().get_string(ID_value), 0, 2);
            return i2string(n) + "==0";
        } else
        ...
        return function_call + "(" + convert(src.op1().op0()) + ")";
    }
