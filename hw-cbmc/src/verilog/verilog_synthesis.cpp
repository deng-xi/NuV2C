...
void verilog_synthesist::convert_module_items(symbolt &symbol)
{
  ...
  forall_symbol_module_map(it, symbol_table.symbol_module_map, module)//{std::cout<<"find out symbols -dx\n";
  ...
  forall_operands(it, symbol.value)//{ std::cout<< "in do the module items -dx \n";
    synth_module_item(
  ...
  for(std::list<exprt>::const_iterator
      ...
      it!=invars.end();
      it++)//{
    trans.invar().copy_to_operands(*it); //std::cout << "in trans.invar().copy -dx \n";}
  ...
  {
    std::cout << "INVAR: " << to_string(*it) <<"\n";
//    warning();
  }

  forall_operands(it, trans.init())
  {
    std::cout << "INIT: " << to_string(*it) << "\n";
//    warning();
  }

  forall_operands(it, trans.trans())
  {
    std::cout << "TRANS: " << to_string(*it) << "\n";
//    warning();
  }
  #endif
  ...
  const optionst &options)
{
  ...
  return verilog_synthesis.typecheck_main(); //typecheck
}
